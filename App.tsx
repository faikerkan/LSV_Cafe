...
import { CafeEvent, Department, EventStatus, EventStatusLabels, LOCATIONS, AppNotification, DepartmentConfig, ResourceConfig, LocationConfig } from './types';
...
import { useEvents, useCreateEvent, useUpdateEvent, useDeleteEvent, useApproveEvent, useRejectEvent } from './hooks/useEvents';
...
const App: React.FC = () => {
  ...
  const handleStatusChange = async (id: string, newStatus: EventStatus) => {
    ...
    try {
        // Özel approve/reject endpoint'lerini kullan
        if (newStatus === EventStatus.APPROVED) {
            ...
            addNotification(
                'Etkinlik Onaylandı',
                `"${event.title}" etkinliği onaylandı.`,
                'approved',
                event.id
            );
            ...
            // Rejection reason sor
            const reason = prompt('Red nedeni (opsiyonel):');
            
            await rejectEventMutation.mutateAsync({ id, reason: reason || undefined });
            
            addToast('success', 'Etkinlik reddedildi.');
            addNotification(
                'Etkinlik Reddedildi',
                `"${event.title}" etkinliği reddedildi.${reason ? ` Neden: ${reason}` : ''}`,
                'rejected',
                event.id
            );
            ...
            // Diğer status değişiklikleri için normal update kullan
            const updatedEvent = { ...event, status: newStatus };
            await api.updateEvent(updatedEvent);
            await fetchEvents();
            
            const actionText = 
                newStatus === EventStatus.COMPLETED ? 'tamamlandı olarak işaretlendi' : 
                'beklemeye alındı';
        ...
        addToast('error', err?.message || 'Durum güncellenemedi.');
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50 text-gray-800">
      ...
      <MobileHeader
        ...
        title={view === "calendar" ? "Takvim" : "Etkinlikler"}
      ...
      />
      {/* Sidebar Navigation */}
      <aside className="hidden lg:flex w-full md:w-64 bg-slate-900 text-white flex-shrink-0 flex flex-col">
        <div className="p-6">
      ...
      <BottomNav
        ...
        onProfileClick={() => setIsMobileMenuOpen(true)}
        isLoggedIn={isLoggedIn}
      />
    </div>
  );
